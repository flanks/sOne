...
_sesamopen = if((EQUAL(count (actionKeys "User6"),0))) then {36} else {(actionKeys "User6") select 0};
_yelpme = if((EQUAL(count (actionKeys "User12"),0))) then {76} else {(actionKeys "User12") select 0};
_radarstuff = if((EQUAL(count (actionKeys "User3"),0))) then {38} else {(actionKeys "User3") select 0};
...
switch (_code) do {
  ...
  case _sesamopen:
	...
	case _yelpme: {
	    if(playerSide in [west] && {vehicle player != player} && {!life_yelp_active} && {((driver vehicle player) == player)}) then {
	    	[] spawn {
