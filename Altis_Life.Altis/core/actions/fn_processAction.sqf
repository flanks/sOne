...
if(_hasLicense) then {
	...
	if(player distance _vendor > 10) exitWith {hint localize "STR_Process_Stay"; 13 cutText ["","PLAIN"]; life_is_processing = false;};
	_IndexNow = -1;
	_ItemMax = [];
	{
		_IndexNow = _IndexNow + 1;
	...
	if(CASH < _cost) exitWith {hint format[localize "STR_Process_License",[_cost] call life_fnc_numberText]; 13 cutText ["","PLAIN"]; life_is_processing = false;};
